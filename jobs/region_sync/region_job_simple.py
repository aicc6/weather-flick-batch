"""
지역 정보 동기화 배치 작업 (간단한 버전)

RegionUnificationJob과 동일한 구조로 BaseJob을 상속받아 구현
config 파라미터 없이 작동하는 버전
"""
...
# BaseJob을 직접 정의 (RegionUnificationJob과 동일한 방식)
class BaseJob:
    """배치 작업 기본 클래스 (간단한 버전)"""
...
class RegionJobSimple(BaseJob):
    """지역 정보 동기화 작업 (간단한 버전)"""
    
    def __init__(self):
        super().__init__()
        self.job_name = "region_job_simple"
        # RegionService 대신 RegionServiceCompatible 사용
    ...
    async def execute(self) -> Dict[str, Any]:
        ...
        self.logger.info("🚀 지역 정보 동기화 작업 시작 (간단한 버전)")
    ...
    def _check_mapping_completeness(self) -> Dict[str, Any]:
        ...
        try:
            ...
            total_regions = self.db_manager.fetch_one(
                "SELECT COUNT(*) as count FROM regions WHERE region_level = 1"
            )['count']
            
            # 메모리 캐시에서 매핑 정보 확인
            kto_mapped = len([k for k in self.region_service._api_mappings_cache.keys() 
                            if 'KTO' in self.region_service._api_mappings_cache.get(k, {})])
            kma_mapped = len([k for k in self.region_service._api_mappings_cache.keys() 
                            if 'KMA' in self.region_service._api_mappings_cache.get(k, {})])
    ...
    def _check_duplicates(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'duplicate_mappings': 0  # 메모리 캐시 사용으로 간소화
            }
...
async def run_region_job_simple():
    """지역 정보 동기화 작업 실행 (간단한 버전)"""
    job = RegionJobSimple()
