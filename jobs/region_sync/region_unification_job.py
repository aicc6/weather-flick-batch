...
from app.services.region_unification_service import get_region_unification_service
from app.core.database_manager import get_db_manager


class RegionUnificationJob(BaseJob):
    """지역 정보 통합 동기화 작업"""
    
    def __init__(self):
        super().__init__()
        self.job_name = "region_unification_job"
        self.region_service = get_region_unification_service()
        self.db_manager = get_db_manager()
        
    async def execute(self) -> Dict[str, Any]:
        """지역 정보 통합 동기화 실행"""
        
        self.logger.info("🚀 지역 정보 통합 동기화 작업 시작")
        ...
        try:
            ...
            self.logger.info(f"✅ 지역 정보 통합 동기화 작업 완료: {results['overall_status']}")
            return results
            
        except Exception as e:
            self.logger.error(f"❌ 지역 정보 통합 동기화 작업 실패: {e}")
    ...
    def _check_region_consistency(self) -> Dict[str, Any]:
        ...
        try:
            ...
            inconsistent_relations = self.db_manager.fetch_all("""
                SELECT ur1.region_code, ur1.region_name, ur2.region_name as parent_name
                FROM unified_regions ur1
                LEFT JOIN unified_regions ur2 ON ur1.parent_region_id = ur2.region_id
                WHERE ur1.parent_region_id IS NOT NULL 
                  AND ur2.region_id IS NULL
            """)
    ...
    def _check_mapping_completeness(self) -> Dict[str, Any]:
        ...
        try:
            ...
            total_regions = self.db_manager.fetch_one(
                "SELECT COUNT(*) as count FROM unified_regions WHERE region_level = 1"
            )['count']
            
            # KTO 매핑된 지역 수
            kto_mapped = self.db_manager.fetch_one("""
                SELECT COUNT(DISTINCT ur.region_id) as count
                FROM unified_regions ur
                JOIN region_api_mappings ram ON ur.region_id = ram.region_id
                WHERE ram.api_provider = 'KTO' AND ur.region_level = 1
            """)['count']
            ...
            kma_mapped = self.db_manager.fetch_one("""
                ...
                FROM unified_regions ur
                JOIN region_api_mappings ram ON ur.region_id = ram.region_id
                WHERE ram.api_provider = 'KMA' AND ur.region_level = 1
            """)['count']
    ...
    def _check_coordinate_validity(self) -> Dict[str, Any]:
        ...
        try:
            ...
            invalid_coordinates = self.db_manager.fetch_all("""
                SELECT region_id, wgs84_latitude, wgs84_longitude
                FROM coordinate_transformations
                WHERE wgs84_latitude < 33.0 OR wgs84_latitude > 38.5
                   OR wgs84_longitude < 124.0 OR wgs84_longitude > 132.0
            """)
            ...
            verified_coordinates = self.db_manager.fetch_one(
                "SELECT COUNT(*) as count FROM coordinate_transformations WHERE is_verified = true"
            )['count']
    ...
    def _check_duplicates(self) -> Dict[str, Any]:
        ...
        try:
            ...
            duplicate_mappings = self.db_manager.fetch_all("""
                SELECT api_provider, api_region_code, COUNT(*) as count
            ...
            if duplicate_mappings:
                duplicate_score -= len(duplicate_mappings) * 10
                issues.append(f"중복 API 매핑: {len(duplicate_mappings)}건")
...
async def run_region_unification_job():
    """지역 정보 통합 동기화 작업 실행"""
    job = RegionUnificationJob()
...
if __name__ == "__main__":
    ...
    async def main():
        print("=== 지역 정보 통합 동기화 배치 작업 ===")
        
        try:
            result = await run_region_unification_job()
            ...
            if result.get('kma_sync'):
                ...
                print(f"- 좌표 변환: {result['kma_sync'].get('transforms_created', 0)}")
