...
def test_multi_api_key_manager():
    ...
    for provider, provider_stats in stats['providers'].items():
        ...
        print(f"  - 총 사용량: {provider_stats['total_usage']}/{provider_stats['total_limit']}")
    ...
    for provider in [APIProvider.KTO, APIProvider.KMA]:
        ...
        if active_key:
            print(f"{provider.value}: {active_key.key[:10]}... (사용량: {active_key.current_usage}/{active_key.daily_limit})")
        else:
...
def test_kto_api_client():
    ...
    try:
        ...
        if result:
            print(f"✅ API 호출 성공: {len(result.get('items', {}).get('item', []))}개 항목 조회")
        else:
            print("❌ API 호출 실패")
