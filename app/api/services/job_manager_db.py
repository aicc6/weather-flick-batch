...
class JobManagerDB:
    ...
    async def get_jobs(
        ...
        try:
            ...
            return JobListResponse(jobs=job_infos, total=total, page=page, size=size)
        finally:
            db.close()
    ...
    async def execute_job(
        ...
        """작업 실행"""
        # DB에 작업 생성 또는 기존 PENDING 작업 업데이트
        db = self.get_db()
        try:
            # 기존 작업이 있는지 확인
            existing_job = (
            ...
            if existing_job:
                # 이미 존재하는 작업인 경우 (PENDING 상태 재실행)
                if existing_job.status == JobStatus.PENDING.value:
                    # PENDING 상태인 경우만 업데이트
                    existing_job.created_by = requested_by or existing_job.created_by
                    existing_job.parameters = parameters
                    db.commit()
                else:
                    # PENDING이 아닌 상태면 실행하지 않음
                    raise ValueError(
                        f"작업 {job_id}는 이미 {existing_job.status} 상태입니다."
                    )
            else:
                # 새로운 작업 생성
    ...
    def _run_job(self, job_id: str, job_type: JobType, parameters: Dict[str, Any]):
        ...
        try:
            ...
            self._send_notification_sync(
                job_id, "job_failed", {"error_message": str(e)}
            )
            ...
            if self.retry_manager:
                try:
                    ...
                    asyncio.run(
                        self.retry_manager.check_and_retry_job(
                            job_id=job_id,
                            error_message=str(e),
    ...
    async def _execute_kto_collection(self, job_id: str, parameters: Dict[str, Any]):
        ...
        try:
            ...
            if parameters.get("force_fail"):
                await self._add_log(
                    job_id, LogLevel.WARNING, "테스트용 강제 실패 요청됨"
                )
    ...
    async def _execute_weather_collection(
        ...
        try:
            ...
            result_summary = {
                ...
                "success_rate": (
                    ((total_regions - failed_regions) / total_regions * 100)
                    if total_regions > 0
                    else 0
                ),
            }
            ...
            await self._update_job_result(job_id, result_summary)
            await self._add_log(
                job_id, LogLevel.INFO, "날씨 데이터 수집 완료", result_summary
            )
    ...
    async def _execute_quality_check(self, job_id: str, parameters: Dict[str, Any]):
        ...
        try:
            ...
            await self._add_log(
                job_id, LogLevel.INFO, "데이터 품질 검사 완료", to_dict_safe(result)
            )
    ...
    async def _execute_weather_change_notification(
        self, job_id: str, parameters: Dict[str, Any]
    ):
        """날씨 변경 알림 실행"""
        try:
            ...
            await self._update_job_progress(
                job_id, 10.0, "날씨 변경 알림 시스템 초기화 중"
            )

            # WeatherChangeNotificationJob 실행
            from jobs.notification.weather_change_notification_job import (
                WeatherChangeNotificationJob,
            )
            from app.core.base_job import JobConfig
            from config.constants import JobType as ConstJobType
            ...
            await self._update_job_progress(
                job_id, 50.0, "날씨 변경 감지 및 알림 처리 중"
            )
            ...
            await self._update_job_result(job_id, to_dict_safe(result))
            await self._add_log(
                job_id, LogLevel.INFO, "날씨 변경 알림 작업 완료", to_dict_safe(result)
            )

        except Exception as e:
            await self._add_log(
                job_id, LogLevel.ERROR, f"날씨 변경 알림 오류: {str(e)}"
            )
            raise
    ...
    async def get_job_logs(
        self, job_id: str, level: Optional[str] = None, page: int = 1, size: int = 100
    ) -> Optional[JobLogsResponse]:
        """작업 로그 조회"""
        db = self.get_db()
        try:
            query = db.query(BatchJobDetail).filter(BatchJobDetail.job_id == job_id)
            ...
            offset = (page - 1) * size
            details = (
                query.order_by(BatchJobDetail.created_at)
                .offset(offset)
                .limit(size)
                .all()
            )
            ...
            return JobLogsResponse(
                job_id=job_id, logs=logs, total=total, page=page, size=size
            )
        finally:
            db.close()

    async def stop_job(
        self, job_id: str, reason: Optional[str] = None, force: bool = False
    ) -> bool:
        """작업 중단"""
        ...
        try:
            ...
            job.completed_at = datetime.utcnow()
            job.error_message = (
                f"작업이 중단됨: {reason}" if reason else "사용자에 의해 중단됨"
            )
            db.commit()
        finally:
            db.close()
    ...
    async def cleanup_old_data(self, days: int) -> Dict[str, Any]:
        ...
        try:
            ...
            # 관련 로그도 삭제
            deleted_logs = (
                db.query(BatchJobDetail)
                .filter(BatchJobDetail.created_at < cutoff_date)
                .delete()
            )
        ...
        logger.info(
            f"Cleaned up {deleted_jobs} old jobs and {deleted_logs} logs older than {days} days"
        )

        return {"deleted_jobs": deleted_jobs, "deleted_logs": deleted_logs}
    ...
    async def _add_log(
        ...
        try:
            ...
            asyncio.create_task(
                websocket_module.send_realtime_log(
                    job_id, level.value, message, details
                )
            )
        except Exception as e:
    ...
    async def _update_job_progress(
        ...
        try:
            ...
            asyncio.create_task(
                websocket_module.send_job_progress_update(
                    job_id, progress, current_step
                )
            )
        except Exception as e:
    ...
    def _send_notification_sync(
        self, job_id: str, event: str, additional_data: Optional[Dict[str, Any]] = None
    ):
        """동기 컨텍스트에서 알림 발송"""
