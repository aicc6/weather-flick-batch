...
class KTODataTransformer(BaseDataTransformer):
    ...
    def _process_coordinates(self, transformed: Dict) -> Dict:
        ...
        if 'longitude' in transformed and 'latitude' in transformed:
            try:
                longitude = float(transformed['longitude']) if transformed['longitude'] else None
                latitude = float(transformed['latitude']) if transformed['latitude'] else None
                ...
                if longitude and latitude:
                    ...
                    if not self._is_valid_korea_coordinate(longitude, latitude):
                        ...
                        self.logger.warning(f"유효하지 않은 좌표: {longitude}, {latitude}")
                else:
...
class KMADataTransformer(BaseDataTransformer):
    ...
    def _transform_current_weather(self, items: List[Dict]) -> List[Dict]:
        ...
        for item in items:
            ...
            if category in self.CATEGORY_MAPPING:
                ...
                weather_data[key][field_name] = self._convert_weather_value(category, value)
    ...
    def _transform_weather_forecast(self, items: List[Dict]) -> List[Dict]:
        ...
        for item in items:
            ...
            if category in self.CATEGORY_MAPPING:
                field_name = self.CATEGORY_MAPPING[category]
                forecast_data[key][field_name] = self._convert_weather_value(category, value)
    ...
    def validate(self, api_provider: str, data: List[Dict]) -> ValidationResult:
        ...
        if api_provider == 'KTO':
            ...
            return ValidationResult(
                ...
                quality_score=50.0,
                errors=[],
                warnings=[{'type': 'unknown_provider', 'message': f'알 수 없는 API 제공자: {api_provider}'}]
            )
...
class DataTransformationPipeline:
    ...
    async def transform_raw_data(self, raw_data_id: str) -> TransformationResult:
        ...
        try:
            ...
            if not raw_data:
                return TransformationResult.error_result("원본 데이터를 찾을 수 없습니다")
            ...
            if not transformer:
                return TransformationResult.error_result(f"지원하지 않는 API 제공자: {api_provider}")
            ...
            transformation_log = {
                ...
                'input_record_count': len(raw_response.get('response', {}).get('body', {}).get('items', {}).get('item', [])),
                'output_record_count': len(processed_data),
                'error_count': len(validation_result.errors),
                'transformation_time_ms': transformation_time_ms,
                'status': 'success' if validation_result.is_valid else 'partial_failure',
                'error_details': {
            ...
            self.logger.info(f"데이터 변환 완료: {raw_data_id} -> {len(processed_data)}건 (품질점수: {validation_result.quality_score:.1f})")
            ...
            try:
                error_log = {
                    ...
                    'transformation_time_ms': transformation_time_ms,
                    'status': 'failure',
                    'error_details': {'errors': [{'type': 'transformation_exception', 'message': str(e)}]}
                }
