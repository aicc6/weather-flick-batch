...
class UnifiedAPIClient:
    ...
    def __init__(self):
        ...
        try:
            ...
            self.logger.warning("Redis 클라이언트를 사용할 수 없습니다. 파일 캐시를 사용합니다.")
    ...
    def _generate_cache_key(self, api_provider: str, endpoint: str, params: Dict) -> str:
        """캐시 키 생성"""
        # 파라미터를 정렬하여 일관된 키 생성
    ...
    async def _execute_api_call(self, 
        ...
        if not base_url:
            raise ValueError(f"{api_provider.value}에 대한 Base URL이 설정되지 않았습니다.")
        ...
        async with self.session.get(url, params=params) as response:
            ...
            if response.status != 200:
                raise ValueError(f"HTTP 오류: {response.status} - {response_text[:200]}...")
            
            # API별 성공 응답 확인
            if api_provider in [APIProvider.KTO, APIProvider.KMA]:
                result_code = response_data.get('response', {}).get('header', {}).get('resultCode')
                if result_code != '00':
    ...
    async def call_api(self, 
        ...
        if isinstance(api_provider, str):
            try:
                ...
                return APIResponse.error_response(f"지원하지 않는 API 제공자: {api_provider}")
        ...
        try:
            if not self.session:
                return APIResponse.error_response("HTTP 세션이 초기화되지 않았습니다. async with 구문을 사용하세요.")
            ...
            self.logger.info(f"API 호출 성공: {api_provider.value}/{endpoint} ({duration_ms}ms)")
            ...
            duration_ms = int((time.time() - start_time) * 1000)
            self.logger.error(f"API 호출 실패: {api_provider.value}/{endpoint} - {e} ({duration_ms}ms)")
