...
class DatabaseConnectionPool:
    ...
    def initialize_sync_pool(self):
        ...
        try:
            ...
            self.stats["sync_pool"]["total_connections"] = (
                self.config.sync_min_connections
            )
            self.logger.info(
                f"동기 커넥션 풀 초기화 완료: {self.config.sync_min_connections}-{self.config.sync_max_connections}"
            )
    ...
    async def initialize_async_pool(self):
        ...
        try:
            ...
            self.stats["async_pool"]["total_connections"] = (
                self.config.async_min_connections
            )
            self.logger.info(
                f"비동기 커넥션 풀 초기화 완료: {self.config.async_min_connections}-{self.config.async_max_connections}"
            )
    ...
    def _start_health_check(self):
        ...
        self._health_check_thread = threading.Thread(
            target=health_check_worker, daemon=True, name="db-health-check"
        )
