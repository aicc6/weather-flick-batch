...
class APITaskManager:
        ...
        def wrapped_task():
            try:
                ...
                if self._is_rate_limit_error(str(e)):
                    self.smart_scheduler.set_api_status(
                        "kto_api", APIStatus.RATE_LIMITED
                    )
                    self.logger.warning(
                        "KTO API 한도 초과 감지 - 다른 작업을 우선 실행합니다"
                    )
                else:
    ...
    def add_weather_task(self, task_func: Callable, task_name: str = None) -> bool:
        ...
        def wrapped_task():
            try:
                ...
                if self._is_rate_limit_error(str(e)):
                    self.smart_scheduler.set_api_status(
                        "weather_api", APIStatus.RATE_LIMITED
                    )
                    self.logger.warning(
                        "기상청 API 한도 초과 감지 - 다른 작업을 우선 실행합니다"
                    )
                else:
    ...
    def add_maintenance_task(
        self, task_func: Callable, task_type: str = "cleanup", task_name: str = None
    ) -> bool:
        """유지보수 작업 추가 (백업, 정리 등)"""
        ...
        return self.smart_scheduler.add_task(
            api_name=task_type, task_func=wrapped_task, priority=priority
        )
    ...
    def get_status_report(self) -> Dict:
        ...
        with self.task_lock:
            ...
            completed_tasks = sum(
                1 for result in self.task_results.values() if result is not None
            )
