...
class SmartAPIScheduler:
    ...
    def add_task(self, api_name: str, task_func: Callable, priority: int = 5, 
    ...
    def set_api_status(self, api_name: str, status: APIStatus, 
        ...
        if status == APIStatus.RATE_LIMITED:
            ...
            self.logger.warning(f"API 한도 초과: {api_name}, 복구 시간: {self.api_rate_limit_reset[api_name]}")
    ...
    def execute_task(self, task: APITask):
        ...
        try:
            ...
            if any(keyword in error_msg for keyword in ['rate limit', 'quota', 'limit exceed']):
                # API 한도 초과
            ...
            if task.retry_count < task.max_retries:
                ...
                self.logger.info(f"작업 재시도 예약: {task.api_name} "
                               f"({task.retry_count}/{task.max_retries}), "
                               f"지연: {task.delay_seconds}초")
                
                # 지연 후 재추가
                threading.Timer(task.delay_seconds, 
