...
class UnifiedKTOClient:
    ...
    async def collect_all_data(self, 
        ...
        async with self.api_client:
            ...
            for content_type in content_types:
                content_name = self.content_types.get(content_type, f'unknown_{content_type}')
                ...
                for area_code in area_codes:
                    try:
                        ...
                        content_results['total_raw_records'] += area_result.get('raw_records', 0)
                        content_results['total_processed_records'] += area_result.get('processed_records', 0)
                ...
                collection_results['content_types_collected'][content_type] = content_results
                collection_results['total_raw_records'] += content_results['total_raw_records']
                collection_results['total_processed_records'] += content_results['total_processed_records']
                
                self.logger.info(f"수집 완료: {content_name} - 원본 {content_results['total_raw_records']}건, 처리 {content_results['total_processed_records']}건")
        
        collection_results['completed_at'] = datetime.utcnow().isoformat()
        
        self.logger.info(f"KTO 데이터 수집 완료: {sync_batch_id} - 총 원본 {collection_results['total_raw_records']}건, 처리 {collection_results['total_processed_records']}건")
        
        return collection_results
    
    async def _collect_area_data(self, 
                               content_type: str, 
                               area_code: str,
                               sync_batch_id: str,
                               store_raw: bool,
                               auto_transform: bool) -> Dict:
        """특정 지역의 특정 타입 데이터 수집"""
    ...
    async def _save_processed_data(self, 
        ...
        try:
            ...
            self.logger.debug(f"처리된 데이터 저장 완료: {target_table} {saved_count}건")
            return saved_count
    ...
    async def collect_category_codes(self, content_type_id: Optional[str] = None) -> List[Dict]:
        """카테고리 코드 정보 수집"""
