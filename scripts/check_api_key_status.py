...
def suggest_solutions(all_limited, next_reset):
    ...
    if all_limited:
        print(f"  📛 모든 API 키가 제한된 상태입니다.")
        
        if next_reset:
            print(f"  ⏳ 대기 방안:")
            print(f"    - {next_reset.strftime('%H:%M')} 이후에 다시 시도")
            print(f"    - 일반적으로 1시간 후 자동 해제됩니다")
        
        print(f"  🔑 추가 API 키 방안:")
        print(f"    - 한국관광공사에서 추가 API 키 발급 신청")
        print(f"    - .env 파일의 KTO_API_KEY에 쉼표로 구분하여 추가")
        
        print(f"  📊 사용량 분산 방안:")
        print(f"    - 수집 지역을 나누어 시간대별로 실행")
        print(f"    - 주요 지역만 우선 수집 후 나머지는 나중에")
    
    else:
        print(f"  ✅ 사용 가능한 API 키가 있습니다.")
        print(f"  🚀 음식점 데이터 수집을 재시도할 수 있습니다.")
        
        print(f"  📈 효율적인 수집 방안:")
        print(f"    - 주요 지역(서울, 부산, 경기, 제주)부터 시작")
        print(f"    - 지역별로 나누어 순차적으로 수집")
        print(f"    - API 제한 발생 시 다른 지역으로 전환")


def check_daily_usage_reset():
    """일일 사용량 리셋 확인"""
    print(f"\n🔄 일일 사용량 리셋 정보:")
    print(f"  - 한국관광공사 API는 일일 기준으로 사용량이 리셋됩니다")
    print(f"  - 보통 자정(00:00)에 리셋되지만, 정확한 시간은 API 제공자에 따라 다를 수 있습니다")
