-- =====================================================
-- current_weather 테이블 raw_data_id 컬럼 추가 마이그레이션
-- 작성일: 2025-07-07
-- 목적: 배치 실행 시 발생하는 raw_data_id 컬럼 오류 수정
-- =====================================================

-- 1. current_weather 테이블에 raw_data_id 컬럼 추가
DO $$ 
BEGIN
    -- raw_data_id 컬럼이 없으면 추가
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'current_weather' 
        AND column_name = 'raw_data_id'
    ) THEN
        ALTER TABLE current_weather 
        ADD COLUMN raw_data_id UUID REFERENCES api_raw_data(id);
        
        RAISE NOTICE 'current_weather 테이블에 raw_data_id 컬럼을 추가했습니다.';
    ELSE
        RAISE NOTICE 'current_weather 테이블에 raw_data_id 컬럼이 이미 존재합니다.';
    END IF;
    ...
    IF NOT EXISTS (
        ...
        RAISE NOTICE 'current_weather 테이블에 updated_at 컬럼을 추가했습니다.';
    ELSE
        RAISE NOTICE 'current_weather 테이블에 updated_at 컬럼이 이미 존재합니다.';
    END IF;
END $$;
...
-- 3. 인덱스 생성 (성능 향상)
CREATE INDEX IF NOT EXISTS idx_current_weather_raw_data_id ON current_weather(raw_data_id);
CREATE INDEX IF NOT EXISTS idx_current_weather_observed_at ON current_weather(observed_at);
...
VALUES (
    'fix_current_weather_raw_data_id', 
    'schema_migration', 
    'success', 
    CURRENT_TIMESTAMP, 
    CURRENT_TIMESTAMP,
    '{"migration": "005_fix_current_weather_raw_data_id", "description": "current_weather 테이블 raw_data_id 컬럼 오류 수정"}'::jsonb
);

COMMENT ON COLUMN current_weather.raw_data_id IS '원본 API 데이터 참조 ID (api_raw_data 테이블 FK)';
COMMENT ON COLUMN current_weather.updated_at IS '레코드 마지막 수정 시간';